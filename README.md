...
5 档角色，与 fmp 后端 RBAC（`sys_role`）对齐（`role` 来自 `data/identity_map.json` 覆盖、`OCL_ADMIN_USER_IDS` 环境变量、或飞书 email 自动建档）：

| 角色 | 能力（车辆预约域）|
|----|------|
| 1 工程师 | 查询可用车辆、预约/取消/归还、查询自己的预约 |
| 2 调度员 | + 审批本组预约、查本组待审批 |
| 3 管理员 | + 跨组审批、系统级操作 |
| 4 司机 | fmp 仅出车监控，bot 不开放约车/审批（仅助手工具）|
| 5 组管理员 | 约车 + 审批本组（≈ 调度员；车组/人员管理在 Web 端）|

fmp 5 角色**非线性**（司机 4 权限比工程师 1 还少、组管理员 5≈调度员而非管理员），故 OCL 用显式 `ROLE_TOOLS` 而非 `role>=min_role` 线性比较。管理员在飞书内指派角色：`设置角色 <open_id> <1-5>`；查看用户：`查看用户`。
