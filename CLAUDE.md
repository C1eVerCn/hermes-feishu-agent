...
角色来源：`data/identity_map.json`（open_id → role）。**与 fmp 后端 RBAC（sys_role）对齐的 5 档**：

- **role=1 工程师** — 查询可用车辆、预约/取消/归还自己的车辆、查询自己的预约
- **role=2 调度员** — +审批本组车辆预约、查询待我审批列表
- **role=3 管理员** — +跨组审批、系统级操作（未来）
- **role=4 司机** — fmp 仅"出车状态监控"，bot 无对应业务工具 → 不开放约车/审批（仅助手工具）
- **role=5 组管理员** — 约车 + 审批本组（车组/人员管理在 Web 端）≈ 调度员

OCL 按角色粗粒度门控工具；后端按 openid/email 细粒度校验（角色 + 分组 + 状态）。
两道闸相互独立。**注意 fmp 5 角色非线性**（司机 4 权限比工程师 1 还少、组管理员 5≈调度员而非
管理员），故 `ocl/permission.py` 用显式 `ROLE_TOOLS`（按角色列工具集），**不**用 role>=min_role
线性比较。fmp 的自定义组角色（如 sys_role.id=100）在 identity_map 归并到 5（组管理员）。

| 角色 | 可调车辆预约工具（== car_tools/register.py 注册名）|
|------|----------|
...
| 3 管理员 | 全部 |
| 4 司机 | get_user_context, get_common_dictionary（仅助手） |

管理员在飞书里指派角色：`设置角色 <open_id> <1-5>`。无自助申请流程。
> 因后端 0617 起 `get_user_context` 不再返回 role（角色移到 sys_user_role 多角色表），且约定不动
> MCP，agent 无法自动同步角色——需管理员按 fmp 角色手动指派（司机→4、组管理员→5）。
...
- 不要新增工具前不在 `ocl/permission.py::ROLE_TOOLS` 给相应角色加该工具（键=register.py 注册名）
