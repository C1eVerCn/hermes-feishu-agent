"""feishu_acl插件的 pre_tool_call钩子测试 — role-based权限拦截（车辆预约域）。"""
import json
import pytest
...
# 普通用户 (role=1)
def test_role1_user_can_use_level1_tool():
    from ocl.session_map import register
    register("feishu_ou_alice", "ou_alice")
    hook = _load_hook()
    assert hook(tool_name="fetch_available_vehicles", session_id="feishu_ou_alice") is None


def test_role1_user_blocked_from_level2_approval_tool():
    """approval_vehicle_reservation 要求 role=2，alice 是1，应拦截。"""
    from ocl.session_map import register
    register("feishu_ou_alice", "ou_alice")
    hook = _load_hook()
    result = hook(tool_name="approval_vehicle_reservation", session_id="feishu_ou_alice")
...
# 调度员 (role=2)
def test_role2_user_can_use_approval_tool():
    from ocl.session_map import register
    register("feishu_ou_bob", "ou_bob")
    hook = _load_hook()
    assert hook(tool_name="approval_vehicle_reservation", session_id="feishu_ou_bob") is None


def test_role2_user_can_use_query_tool():
    from ocl.session_map import register
    register("feishu_ou_bob", "ou_bob")
    hook = _load_hook()
    assert hook(tool_name="fetch_available_vehicles", session_id="feishu_ou_bob") is None
...
def test_role3_admin_can_access_all_tools():
    ...
    hook = _load_hook()
    for tool in ["fetch_available_vehicles", "approval_vehicle_reservation",
                 "fetch_user_approval"]:
        assert hook(tool_name=tool, session_id="feishu_ou_carol") is None, \
...
def test_hook_passes_when_no_session_id():
    ...
    hook = _load_hook()
    assert hook(tool_name="approval_vehicle_reservation", session_id="") is None


def test_hook_passes_when_user_not_in_map():
    """session 找不到对应 user，让 Layer2 处理，fail-open。"""
    hook = _load_hook()
    assert hook(tool_name="approval_vehicle_reservation", session_id="unknown_session") is None
...
def test_hook_passes_on_permission_check_exception(monkeypatch):
    ...
    hook = _load_hook()
    assert hook(tool_name="approval_vehicle_reservation", session_id="feishu_ou_alice") is None
