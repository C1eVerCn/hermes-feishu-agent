...
**附：角色档位从 3 档扩到 5 档**，机器人侧已（2026-06-26）把角色模型对齐到 fmp `sys_role`，
按实测 `sys_role_menu` 矩阵镜像成显式 per-role 工具集（非线性）：

| roleId | 名称 | 机器人角色 | 约车能力（与 fmp sys_role_menu 一致）|
|--------|------|-----------|------|
| 1 | 工程师 | 1 工程师 | 约车/查/取消/还，不能审批 |
| 2 | 调度员 | 2 调度员 | + 审批本组 |
| 3 | 管理员 | 3 管理员 | 全部 |
| 4 | 司机 | 4 司机 | fmp 仅"出车监控"→ bot 不开放约车/审批（仅助手工具） |
| 5 | 组管理员 | 5 组管理员 | 约车 + 审批本组（≈ 调度员；车组/人员管理在 Web 端） |

机器人按角色显式列工具（`ocl/permission.py::ROLE_TOOLS`），**不**用 `role>=min_role` 线性比较
（否则司机 4≥2 会误授审批、组管理员 5≥3 会误授管理员）。fmp 自定义组角色（如 `sys_role.id=100`）
在 identity_map 归并到 5。

> 仍待后端的只是 **roleIds 回传**（上方两个方案）——补上后机器人即可自动同步，无需管理员手动
> `设置角色`。当前因 `getUserContext` 不返回角色，司机(4)/组管理员(5) 需管理员手动按上表指派。
