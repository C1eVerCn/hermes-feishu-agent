...
Tier-2 现支持 cancel/return/approve：`intent_router` 额外抽 `vehicle_no`/`reservation_id`/`approved`/
`review_comment`，`fast_path.run_mutation` 确定性调对应 handler。安全约束：

- **必须有明确识别符**（vehicle_no 或 reservation_id）才执行，否则返回 None → 落 agent 追问。
- approve 还要求 `approved` 布尔明确（批准/驳回），否则落 agent。
- 角色门控（approve 需 role≥2）+ 后端按 emailAddress + 归属细粒度鉴权（取消只能取消自己待审批、
  审批只能审本组）双重把关。
- **直接执行、无二次确认**（与 query 一致、与 agent 路径同等风险，但参数是确定性抽取而非 LLM 现编）。
  若需"先确认再执行"的更稳 UX，可后续加一张确认卡（参照 booking 的 CONFIRM）。
- 防误判：Tier-1 `is_booking_intent` 加了动作守卫——句中含 取消/归还/审批 等词时不判为 booking
  （即使含车辆编号），把 "把PNV332取消" 正确交给 Tier-2 而非误进约车 FSM。

### 手机号作为第二识别符（2026-06-25 新增）

- 邮箱仍是**主识别符**：上游 fmp 每个 `@Tool` 都以 `emailAddress` 鉴权、签名里**没有 mobile**，
  注入 mobile 会 `TypeError`（同 openId）。故 mobile **不下传上游**，仅作本系统识别符。
- 本系统全链路支持：`identity_map` 存 mobile、`identity.mobile_of`/`open_id_of_mobile` 解析、
  `identity_admin` 存/查、`CallerIdentity` 携带、`查看用户 <手机号>` 按手机号查人、
  `设置手机/绑定手机 <open_id> <号码>` 管理员命令。
- 来源：飞书 contact v3 默认无 mobile 权限 → mobile 目前靠管理员命令 / 手动改 identity_map 写入
  （`get_user_context` 后端能返回 mobile，后续可用它回填）。
