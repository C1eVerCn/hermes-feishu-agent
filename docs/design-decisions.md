...
### 4. 为什么权限审批在 handler.py 里用关键词匹配而非 Agent

**决策：** 权限申请/审批用正则硬编码匹配，不走 Agent。

**理由：**
- 可靠性：权限指令是"批准 ou_xxx write"这种格式化的管理命令，不需要 LLM 的理解
- 延迟：关键词匹配 < 1ms，Agent 调用 2-15s
- 安全性：Agent 调用可能被 prompt injection 影响，关键词匹配不会
- 管理员通知：需要调用 `sender.send_to_user()`，这是确定性操作，不需要 LLM

**何时该走 Agent：** 只有当指令需要语义理解时（如"帮我查一下最近有哪些订单需要发货"），才值得付出 2-15s 的延迟。
...
1. **Plugin 依赖 `~/.hermes/config.yaml` 配置：** 如果配置文件被误改或删除，Layer 1 失效
...
4. **权限申请无重复防护：** 用户可以反复申请同一权限，每次都会通知管理员
5. **thread-local 仍有残留风险：** Layer 2 的 thread-local 仍然是概率性防护
